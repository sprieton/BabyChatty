...
from utils import GenConfig as cfg, AnalysisConfig as acfg
...
def compute_quality_metrics(df: pd.DataFrame, corpus_name: str) -> dict:
    ...
    out["file"] = str(acfg.corpora.get(corpus_name, acfg.parquet_file))
...
def compare_corpora():
    ...
    for corpus_name, path in acfg.corpora.items():
        if not path.exists():
    ...
    summary.to_csv(acfg.out_path, index=False)
    print(f"Saved corpus comparison to: {acfg.out_path}")
    print(summary.to_string(index=False))


def clean_final_corpus():
    """Clean the selected final corpus and save a cleaned parquet."""
    df = pd.read_parquet(acfg.parquet_file).copy()
    ...
    non_clinical_url_regex = "|".join(acfg.non_clinical_url_patterns)
    non_clinical_title_regex = "|".join(acfg.non_clinical_title_patterns)
    ...
    cleaned_path = acfg.parquet_file
    df.to_parquet(cleaned_path, index=False)

    metrics, checked_df = compute_quality_metrics(df, acfg.corpus_name)
    pd.DataFrame([metrics]).to_csv(acfg.corpus_quality_path, index=False)

    print(f"Original docs: {before}")
    print(f"Cleaned docs: {after}")
    print(f"Saved cleaned corpus to: {cleaned_path}")
    print(f"Saved final quality summary to: {acfg.corpus_quality_path}")
...
def export_manual_review_sample(n=30, seed=42):
    ...
    df = pd.read_parquet(acfg.cleaned_path)
